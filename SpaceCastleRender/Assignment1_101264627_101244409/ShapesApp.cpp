...
void ShapesApp::Draw(const GameTimer& gt)
{
    ...
    mCommandList->ClearRenderTargetView(CurrentBackBufferView(), Colors::Purple, 0, nullptr);
...
void ShapesApp::BuildShapeGeometry()
{
    ...
    GeometryGenerator::MeshData box = geoGen.CreateBox(5.0f, 0.8f, 3.0f, 3);
    GeometryGenerator::MeshData grid = geoGen.CreateGrid(30.0f, 30.0f, 60, 40);
    GeometryGenerator::MeshData sphere = geoGen.CreateSphere(0.5f, 20, 20);
    GeometryGenerator::MeshData cylinder = geoGen.CreateCylinder(0.7f, 0.7f, 4.0f, 20, 20);
    GeometryGenerator::MeshData cone = geoGen.CreateCone(1.0f, 8.0f, 20, 20);
    GeometryGenerator::MeshData triPrism = geoGen.CreateTriPrism(0.5f, 0.5f, 3.0f, 20);
    GeometryGenerator::MeshData planet = geoGen.CreatePlanet(0.5f, 20, 20);
    GeometryGenerator::MeshData diamond = geoGen.CreateDiamond(0.5f);
    GeometryGenerator::MeshData pyramid = geoGen.CreatePyramid(0.5f, 3.0f, 20);
    GeometryGenerator::MeshData gradPyr = geoGen.CreateGradPyr(0.7f, 0.2f, 2.0f, 20);
    ...
    UINT planetVertexOffset = triPrismVertexOffset + (UINT)triPrism.Vertices.size();
    UINT diamondVertexOffset = planetVertexOffset + (UINT)planet.Vertices.size();
    UINT pyramidVertexOffset = diamondVertexOffset + (UINT)diamond.Vertices.size();
    UINT gradPyrVertexOffset = pyramidVertexOffset + (UINT)pyramid.Vertices.size();
    ...
    UINT planetIndexOffset = triPrismIndexOffset + (UINT)triPrism.Indices32.size();
    UINT diamondIndexOffset = planetIndexOffset + (UINT)planet.Indices32.size();
    UINT pyramidIndexOffset = diamondIndexOffset + (UINT)diamond.Indices32.size();
    UINT gradPyrIndexOffset = pyramidIndexOffset + (UINT)pyramid.Indices32.size();
    ...
    SubmeshGeometry planetSubmesh;
    planetSubmesh.IndexCount = (UINT)planet.Indices32.size();
    planetSubmesh.StartIndexLocation = planetIndexOffset;
    planetSubmesh.BaseVertexLocation = planetVertexOffset;
    ...
    SubmeshGeometry gradPyrSubmesh;
    gradPyrSubmesh.IndexCount = (UINT)gradPyr.Indices32.size();
    gradPyrSubmesh.StartIndexLocation = gradPyrIndexOffset;
    gradPyrSubmesh.BaseVertexLocation = gradPyrVertexOffset;
    ...
    auto totalVertexCount =
        ...
        planet.Vertices.size() +
        diamond.Vertices.size() +
        pyramid.Vertices.size() +
        gradPyr.Vertices.size();
    ...
    for (size_t i = 0; i < planet.Vertices.size(); ++i, ++k)
    {
        vertices[k].Pos = planet.Vertices[i].Position;
    ...
    for (size_t i = 0; i < gradPyr.Vertices.size(); ++i, ++k)
    {
        vertices[k].Pos = gradPyr.Vertices[i].Position;
        vertices[k].Color = XMFLOAT4(DirectX::Colors::Tomato);
    }
    ...
    indices.insert(indices.end(), std::begin(planet.GetIndices16()), std::end(planet.GetIndices16()));
    indices.insert(indices.end(), std::begin(diamond.GetIndices16()), std::end(diamond.GetIndices16()));
    indices.insert(indices.end(), std::begin(pyramid.GetIndices16()), std::end(pyramid.GetIndices16()));
    indices.insert(indices.end(), std::begin(gradPyr.GetIndices16()), std::end(gradPyr.GetIndices16()));
    ...
    geo->DrawArgs["planet"] = planetSubmesh;
    geo->DrawArgs["diamond"] = diamondSubmesh;
    geo->DrawArgs["pyramid"] = pyramidSubmesh;
    geo->DrawArgs["gradPyr"] = gradPyrSubmesh;
...
void ShapesApp::BuildRenderItems()
{
    ...
    XMStoreFloat4x4(&sphereRitem->World, XMMatrixScaling(8.0f, 8.0f, 8.0f));
    ...
    XMStoreFloat4x4(&coneRitem->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 5.0f, 0.0f) * XMMatrixRotationX(3.144f));
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (int j = 0; j < 5; j++)
        {
            auto grPRitem = std::make_unique<RenderItem>();
            auto grPRitem2 = std::make_unique<RenderItem>();

            XMMATRIX grPWorld = XMMatrixTranslation(-4.0f + i * 8.0f, 15.5f, -2.0f + j * 1.0f);
            XMMATRIX grPWorld2 = XMMatrixTranslation(-4.0f + i * 8.0f, 15.5f, -2.0f + j * 1.0f);

            XMStoreFloat4x4(&grPRitem->World, grPWorld * XMMatrixScaling(0.5f, 0.3f, 0.5f));
            grPRitem->ObjCBIndex = objCBIndex++;
            grPRitem->Geo = mGeometries["shapeGeo"].get();
            grPRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            grPRitem->IndexCount = grPRitem->Geo->DrawArgs["gradPyr"].IndexCount;
            grPRitem->StartIndexLocation = grPRitem->Geo->DrawArgs["gradPyr"].StartIndexLocation;
            grPRitem->BaseVertexLocation = grPRitem->Geo->DrawArgs["gradPyr"].BaseVertexLocation;

            XMStoreFloat4x4(&grPRitem2->World, grPWorld2 * XMMatrixScaling(0.5f, 0.3f, 0.5f) * XMMatrixRotationY(1.572f));
            grPRitem2->ObjCBIndex = objCBIndex++;
            grPRitem2->Geo = mGeometries["shapeGeo"].get();
            grPRitem2->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            grPRitem2->IndexCount = grPRitem2->Geo->DrawArgs["gradPyr"].IndexCount;
            grPRitem2->StartIndexLocation = grPRitem2->Geo->DrawArgs["gradPyr"].StartIndexLocation;
            grPRitem2->BaseVertexLocation = grPRitem2->Geo->DrawArgs["gradPyr"].BaseVertexLocation;

            mAllRitems.push_back(std::move(grPRitem));
            mAllRitems.push_back(std::move(grPRitem2));
        }

      /*  for (int j = 0; j < 2; j++)
        {
            auto diaRitem = std::make_unique<RenderItem>();
            auto diaRitem2 = std::make_unique<RenderItem>();
            auto diaRitem3 = std::make_unique<RenderItem>();

            XMMATRIX diaWorld = XMMatrixTranslation(-4.0f + i * 8.0f, 5.5f, -2.0f + j * 1.0f);
            XMMATRIX diaWorld2 = XMMatrixTranslation(-4.0f + i * 8.0f, 5.5f, -2.0f + j * 1.0f);
            XMMATRIX diaWorld3 = XMMatrixTranslation(-4.0f + i * 8.0f, 5.5f, -2.0f + j * 1.0f);

            XMStoreFloat4x4(&diaRitem->World, diaWorld * XMMatrixScaling(0.5f, 0.3f, 0.5f));
            diaRitem->ObjCBIndex = objCBIndex++;
            ...
            XMStoreFloat4x4(&diaRitem2->World, diaWorld2 * XMMatrixScaling(1.0f, 1.0f, 1.0f));
            diaRitem2->ObjCBIndex = objCBIndex++;
            diaRitem2->Geo = mGeometries["shapeGeo"].get();
            diaRitem2->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            diaRitem2->IndexCount = diaRitem2->Geo->DrawArgs["diamond"].IndexCount;
            diaRitem2->StartIndexLocation = diaRitem2->Geo->DrawArgs["diamond"].StartIndexLocation;
            diaRitem2->BaseVertexLocation = diaRitem2->Geo->DrawArgs["diamond"].BaseVertexLocation;

            XMStoreFloat4x4(&diaRitem3->World, diaWorld3 * XMMatrixScaling(1.0f, 1.0f, 1.0f));
            diaRitem3->ObjCBIndex = objCBIndex++;
            diaRitem3->Geo = mGeometries["shapeGeo"].get();
            diaRitem3->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            diaRitem3->IndexCount = diaRitem3->Geo->DrawArgs["diamond"].IndexCount;
            diaRitem3->StartIndexLocation = diaRitem3->Geo->DrawArgs["diamond"].StartIndexLocation;
            diaRitem3->BaseVertexLocation = diaRitem3->Geo->DrawArgs["diamond"].BaseVertexLocation;

            mAllRitems.push_back(std::move(diaRitem));
            mAllRitems.push_back(std::move(diaRitem2));
            mAllRitems.push_back(std::move(diaRitem3));
        ...
        XMMATRIX boxWorld = XMMatrixTranslation(0.0f, -2.0f + i * 4.0f, -3.0f);
        XMMATRIX boxWorld2 = XMMatrixTranslation(0.0f, -2.0f + i * 4.0f, -3.0f);
        XMMATRIX cylWorld = XMMatrixTranslation(-2.0f + i * 4.0f, 3.0f, 2.0f);
        XMMATRIX cylWorld2 = XMMatrixTranslation(-2.0f + i * 4.0f, 3.0f, -2.0f);
        XMMATRIX cylWorld3 = XMMatrixTranslation(-0.0f , 0.0f, -0.0f);
        XMMATRIX cylWorld4 = XMMatrixTranslation(-10.5f + i * 21.0f, 0.0f, -0.0f);
        XMMATRIX cylWorld5 = XMMatrixTranslation(-10.5f + i * 21.0f, 0.0f, -0.0f);
        XMMATRIX sphereWorld = XMMatrixTranslation(-3.0f + i * 6.0f, 0.0f, -0.0f);
        XMMATRIX sphereWorld2 = XMMatrixTranslation(-3.0f + i * 6.0f, 0.0f, -0.0f);
        XMMATRIX pyramidWorld = XMMatrixTranslation(-3.0f + i * 6.0f, 2.5f, -0.0f);
        XMMATRIX pyramidWorld2 = XMMatrixTranslation(-3.0f + i * 6.0f, 2.5f, -0.0f);
        XMMATRIX diaWorld = XMMatrixTranslation(-2.0f + i * 4.0f, 5.0f, 2.0f);
        XMMATRIX diaWorld2 = XMMatrixTranslation(-2.0f + i * 4.0f, 5.0f, -2.0f);


        XMStoreFloat4x4(&boxRitem->World, boxWorld * XMMatrixRotationX(1.572f));
        ...
        XMStoreFloat4x4(&boxRitem2->World, boxWorld2 * XMMatrixRotationX(1.572f) * XMMatrixRotationY(1.572f));
        ...
        XMStoreFloat4x4(&cylRitem2->World, cylWorld2);
        cylRitem2->ObjCBIndex = objCBIndex++;
        ...
        XMStoreFloat4x4(&cylRitem3->World, cylWorld3 * XMMatrixScaling(0.8f, 6.0f, 0.8f) * XMMatrixRotationX(1.572f) * XMMatrixRotationY(i * 1.572f) );
        cylRitem3->ObjCBIndex = objCBIndex++;
        ...
        XMStoreFloat4x4(&cylRitem4->World, cylWorld4 * XMMatrixScaling(0.8f, 5.0f, 0.8f) * XMMatrixRotationX(1.572f) * XMMatrixRotationY(2.361f));
        cylRitem4->ObjCBIndex = objCBIndex++;
        ...
        XMStoreFloat4x4(&cylRitem5->World, cylWorld5 * XMMatrixScaling(0.8f, 5.0f, 0.8f) * XMMatrixRotationX(1.572f) * XMMatrixRotationY(-2.361f));
        cylRitem5->ObjCBIndex = objCBIndex++;
        ...
        XMStoreFloat4x4(&sphereRitem->World, sphereWorld * XMMatrixScaling(4.0f, 4.0f, 4.0f) *  XMMatrixRotationY(1.572f));
        ...
        XMStoreFloat4x4(&sphereRitem2->World, sphereWorld2 * XMMatrixScaling(4.0f, 4.0f, 4.0f));
        ...
        XMStoreFloat4x4(&pyramidRitem->World, pyramidWorld* XMMatrixScaling(4.0f, 1.0f, 4.0f)* XMMatrixRotationX(3.144f)*  XMMatrixRotationY(1.572f));
        ...
        XMStoreFloat4x4(&pyramidRitem2->World, pyramidWorld2* XMMatrixScaling(4.0f, 1.0f, 4.0f) * XMMatrixRotationX(3.144f));
        pyramidRitem2->ObjCBIndex = objCBIndex++;
        pyramidRitem2->Geo = mGeometries["shapeGeo"].get();
        pyramidRitem2->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
        ...
        XMStoreFloat4x4(&diaRitem->World, diaWorld * XMMatrixScaling(1.0f, 1.0f, 1.0f));
        diaRitem->ObjCBIndex = objCBIndex++;
        diaRitem->Geo = mGeometries["shapeGeo"].get();
        diaRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
        diaRitem->IndexCount = diaRitem->Geo->DrawArgs["diamond"].IndexCount;
